...
public class SimpleInstrumenter {
  ...
  public byte[] InstrumentOneClass(String classname, byte[] input_class) {
    ...
    try {
      fos_transform =
          new FileOutputStream(new File(test_visual + "/" + "transform_" + filename + ".class"));
      fos_transform.write(b);
      fos_original =
          new FileOutputStream(new File(test_visual + "/" + "original_" + filename + ".class"));
...
class SimpleMethodAdapter extends MethodVisitor {
  ...
  protected void InstrumentMethodInsn(
