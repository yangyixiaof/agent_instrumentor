...
class UpdateableZipFile(ZipFile):
    ...
    def writestr(self, zinfo_or_arcname, _, compress_type=None):
...
def TestUpdateableZipFile(apk_file_path):
    if not os.path.exists("test_pool"):
      os.makedirs("test_pool")
    _, file_str = os.path.split(apk_file_path)
    optr_file = "test_pool/" + file_str
    shutil.copyfile(apk_file_path, optr_file)
    f = zipfile.ZipFile(apk_file_path, 'r')
    for file in f.namelist():
        pattern = re.compile(r'classes([1-9]*)\.dex')
        if_match = re.match(pattern, file)
        if if_match:
            with UpdateableZipFile(optr_file, "a") as o:
                o.write("nature_view.jpg", file)
            print("Replacing content of .dex " + file + "# to nature_view.jpg")
'''
unzip_directory = "YYX_Unzip"
if os.path.exists('YYX_Unzip'):
   shutil.rmtree(unzip_directory)
os.mkdir(unzip_directory)
f = zipfile.ZipFile("QQ_762.apk", 'r')
for file in f.namelist():
    print(file)
    try:
        f.extract(file, unzip_directory + "/")
    except:
        pass

print("Current work directory:" + sys.path[0])
with UpdateableZipFile("QQ_762.apk", "a") as o:
    # Overwrite a file with a string
    # o.writestr("word/document.xml", "Some data")

    # exclude an exiting file from the zip
    # o.remove_file("classes.dex")

    # Write a new file (with no conflict) to the zp
    # o.writestr("new_file", "more data")

    # Overwrite a file with a file
    o.write("D:/TempliyFiles/nature_view.jpg", "classes.dex")

print("test over!!!")
#z = zipfile.ZipFile('QQ_762.apk', 'r')
#for i in z.infolist():
#    print(i, i.header_offset)
'''



def InstrumentDex(apk_file_path):
    if not os.path.exists("operation_pool"):
        os.makedirs("operation_pool")
    _, file_str = os.path.split(apk_file_path)
    ...
    for file_name in f.namelist():
        ...
        if if_match:
            ...
            with UpdateableZipFile(optr_file, "a") as o:
                instrumented_dex = "DexInstrumentDirectory/" + filename + "-dex2jar-jar2dex.dex"
                if os.path.exists(instrumented_dex):
                    o.write(instrumented_dex, file_name)
                    print("Replacing .dex " + file_name)
...
if __name__ == "__main__":
    print(sys.argv[0])
    print(sys.argv[1])
    # TestUpdateableZipFile("QQ_762.apk")
    InstrumentDex(sys.argv[1])
